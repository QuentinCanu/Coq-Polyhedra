...
Definition update
  ...
  let M' := IFold.ifold (fun k M' =>
    ...
    let Ik := I.[k] in
    let M'Ik := BigQUtils.bigQ_add_arr M.[Ik] (BigQUtils.bigQ_scal_arr v.[k] Au) in
    M'.[Ik <- M'Ik]) (length I) M' in
  let M'r := BigQUtils.bigQ_add_arr M.[Is] (BigQUtils.bigQ_scal_arr vr Au) in
  let M' := M'.[r <- M'r] in
  M'.
